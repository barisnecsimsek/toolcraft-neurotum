...
describe("Toolcraft template app acceptance coverage", () => {
  ...
  it("publishes control order targets for app schema tests", () => {
    expect(getToolcraftControlOrderTargets(appSchema)).toEqual([
      ...
      "particle.tintColor",
      "particle.groupMode",
      "particle.groupColor1",
      "particle.groupColor2",
      "particle.groupColor3",
      "particle.groupColor4",
  ...
  it("declares the Particle Grid reference runtime clone", () => {
  ...
  it("requires explicit reference behavior coverage in reference-runtime-clone mode", () => {
    const acceptanceWithoutReferenceCoverage = appAcceptance.map(
      ({ referenceCoverage: _referenceCoverage, ...entry }) => entry,
    );

    expect(
      validateToolcraftAcceptanceCoverage(appSchema, acceptanceWithoutReferenceCoverage, {
        behaviorCoverage: [
          "canvas-sizing",
          "control-mapping",
          "renderer-state",
  ...
  it("accepts reference-runtime-clone mode only when required reference behavior is test-backed", () => {
    ...
    expect(
      validateToolcraftAcceptanceCoverage(
        appSchema,
        [
          ...acceptanceWithoutReferenceCoverage,
          {
            automated: true,
            automatedTestName: "reference canvas size matches legacy renderer",
            browser: true,
