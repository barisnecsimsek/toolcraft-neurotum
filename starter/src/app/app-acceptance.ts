...
export const appTransferMode: ToolcraftTransferMode = {
  ...
  referenceFeatureInventory: [
    {
      acceptanceId: "canvas.renderScale",
      behaviorEvidence:
        "The shader renders against frame.output dimensions and derives every grid cell from normalized output UV coordinates.",
      featureName: "Output-relative grid",
      id: "output-relative-grid",
      referenceBehavior:
        "Particle cells scale with output width and height while Columns and Rows remain logical counts.",
      sourceEvidence:
        "render() reads frame.output width/height and fs_main derives cellCoord from outputUv_in times gridDims.",
      status: "toolcraft-native",
      toolcraftMapping:
        "Editable Toolcraft canvas sizing and Resolution scale change backing pixels while the shader keeps normalized grid geometry.",
    },
    {
      acceptanceId: "source.image",
      behaviorEvidence:
        "The fragment shader samples inputTex at each grid-cell center through sampleInput(centerUV).",
      featureName: "Input image sampling",
      id: "input-image-sampling",
      referenceBehavior:
        "One input texture supplies color and luminance for every particle cell.",
      sourceEvidence:
        "WGSL binding 2 declares inputTex and fs_main samples it at centerUV before luminance calculation.",
      status: "toolcraft-native",
      toolcraftMapping:
        "The built-in image FileDrop supplies source.image and runtime rotate/flip metadata to the WebGL texture sampler.",
    },
    {
    ...
    },
    {
      acceptanceId: "renderer.particle-grid",
      behaviorEvidence:
        "The reference switches between sampled color, Tint, four luminance groups, and four hue/saturation groups.",
      featureName: "Color mapping",
      id: "color-mapping",
      referenceBehavior:
        "Color Mode selects Original or Tint when grouping is Off; grouping overrides it with four authored colors.",
      sourceEvidence:
        "rgbToHue, rgbSaturation, colorMode, groupMode, and groupColor1..4 branches define particleColor.",
      status: "ported",
      toolcraftMapping:
        "Color Mapping selects and colors feed the matching GLSL classification branches and uniforms.",
    },
    {
      acceptanceId: "renderer.particle-grid",
      behaviorEvidence:
        "Bright cells use a deterministic hash chance and a repeated circular sub-grid controlled by density and dot size.",
      featureName: "Bright-cell dot pattern",
      id: "dot-pattern",
      referenceBehavior:
        "A stable subset of cells with luminance at least 0.5 receives the configured dot texture.",
      sourceEvidence:
        "hash21, dotChance, dotPatternBg, dotDensity, and dotSize determine finalParticleColor in fs_main.",
      status: "ported",
      toolcraftMapping:
        "The Dot Pattern section drives the same hash threshold and circular smoothstep math in GLSL.",
    },
    {
      acceptanceId: "renderer.particle-grid",
      behaviorEvidence:
        "fs_main mixes backgroundColor and finalParticleColor by the rectangular particle mask, including alpha.",
      featureName: "Background composite",
      id: "background-composite",
      referenceBehavior:
        "Particles composite over a configurable RGBA background.",
      sourceEvidence:
        "The final reference lines mix bg and finalParticleColor for both RGB and alpha.",
      status: "toolcraft-native",
      toolcraftMapping:
        "Toolcraft Background controls supply the shader color and standard Include behavior for transparent PNG output.",
    },
  ],
  referenceName: "Figma Particle Grid Effect shader",
  referenceStudy: {
    behaviorEvidence:
      "Source inspection verified cell-center sampling, luminance geometry, color/group branches, deterministic dot pattern, and final alpha composite.",
    referenceLocation: "../inspiration/figma-shaders/particle-grid-effect.js",
    reproductionSteps:
      "Read the complete setup(), render(), WGSL shader, and defineProperties block; mapped every output-affecting branch and property to Toolcraft acceptance coverage.",
    sourceEvidence:
      "Inspected the complete 387-line Figma shader source, including GPU resources, uniforms, fragment equations, render pass, and 22 property definitions.",
    sourceOnlyReason:
      "The artifact cannot run locally because the required figma:shaders module and Figma shader host runtime are unavailable in a standalone browser.",
...
export const appProductReadiness: ToolcraftProductReadiness = {
  mode: "product",
  productName: "Particle Grid Effect",
  productSummary:
    "Reconstructs an uploaded image as a configurable luminance-driven particle grid.",
  requestedBehavior:
    "Replace Bayer Dither with the supplied Figma Particle Grid shader behavior, preserving its grid, particle shape, color grouping, dot pattern, background, and image export controls.",
};

const gridTargets = [
  ["particle.columns", "slider"],
...
const colorTargets = [
  ["particle.colorMode", "select"],
  ["particle.tintColor", "color"],
  ["particle.groupMode", "select"],
  ["particle.groupColor1", "color"],
  ["particle.groupColor2", "color"],
  ["particle.groupColor3", "color"],
  ["particle.groupColor4", "color"],
] as const;

const dotTargets = [
  ["particle.dotChance", "slider"],
  ["particle.dotPatternBackground", "color"],
  ["particle.dotDensity", "slider"],
  ["particle.dotSize", "slider"],
] as const;

export const appAcceptance: readonly ToolcraftComponentAcceptance[] = [
  {
    automated: true,
    automatedTestName: "resolution scale control changes canvas backing pixels",
    browser: true,
    browserTestName: "browser: resolution scale changes backing pixels",
    componentType: "slider",
    evidence: "rendered-pixels",
    expectedObservable:
      "Changing Resolution scale changes the live canvas backing pixel dimensions without changing product geometry.",
    fixture: "default editable output canvas",
    id: "canvas.renderScale",
    kind: "control",
    referenceCoverage: "canvas-sizing",
    target: "canvas.renderScale",
    userAction: "Drag Resolution scale and inspect the backing canvas pixel dimensions.",
  },
  {
    automated: true,
    automatedTestName: "source image control declares the complete media lifecycle",
    browser: true,
    browserTestName: "browser: source image lifecycle updates Particle Grid output",
    componentType: "fileDrop",
    evidence: "media-lifecycle",
    expectedObservable:
      "Uploading a source image renders particle-grid pixels; rotate and flip update runtime media transform metadata consumed by preview and export; Clear and Reset restore the neutral canvas.",
    fixture: "asymmetric 320 by 180 SVG source image",
    id: "source.image",
    kind: "control",
    referenceCoverage: "control-mapping",
    target: "source.image",
    userAction:
      "Upload a source image, rotate it, flip it horizontally and vertically, clear it, upload again, then use section and global Reset controls.",
  },
  ...gridTargets.map(([target, componentType], index) => ({
    automated: true,
    automatedTestName: "grid controls map the reference ranges",
    browser: true,
    browserTestName: "browser: grid controls change Particle Grid geometry",
    componentType,
    ...
    ...(index === 0 ? { referenceCoverage: "control-mapping" as const } : {}),
    target,
  ...
  ...shapeTargets.map((target) => ({
    ...
    kind: "control" as const,
    target,
    userAction: "Drag the corresponding Particle Shape slider and observe live output pixels.",
  })),
  ...colorTargets.map(([target, componentType]) => ({
    automated: true,
    automatedTestName: "color controls map the reference modes and palette",
    browser: true,
    browserTestName: "browser: color mapping controls change particle colors",
    componentType,
    evidence: "rendered-pixels" as const,
    expectedObservable:
      "Original, Tint, brightness grouping, color grouping, and all authored colors change the visible particle colors through the reference branches.",
    fixture: "saturated red, yellow, green, blue, gray, and luminance gradient image",
    id: target,
    kind: "control" as const,
    ...(target === "particle.colorMode"
      ? { optionCoverage: ["original", "tint"] as const }
      : target === "particle.groupMode"
        ? { optionCoverage: ["off", "brightness", "color"] as const }
        : {}),
    target,
    userAction: "Change the corresponding Color Mapping control and inspect particle pixels.",
  })),
  ...dotTargets.map(([target, componentType]) => ({
    automated: true,
    automatedTestName: "dot pattern controls map the reference ranges",
    browser: true,
    browserTestName: "browser: dot pattern controls change bright-cell texture",
    componentType,
    evidence: "rendered-pixels" as const,
    expectedObservable:
      "Changing Chance, Pattern background, Density, or Dot size changes deterministic texture inside bright particle cells.",
    fixture: "uniform bright source image with enough cells to expose the deterministic hash",
    id: target,
    kind: "control" as const,
    target,
    userAction: "Change the corresponding Dot Pattern control and inspect bright-cell pixels.",
  })),
  {
    automated: true,
    automatedTestName: "background inclusion controls preview and PNG alpha",
    browser: true,
    browserTestName: "browser: background inclusion changes preview and PNG alpha",
    componentType: "switch",
    evidence: "rendered-pixels",
    expectedObservable:
      "Turning Include off hides the live preview product background and makes non-particle PNG output transparent; video output keeps the product background, while turning Include on restores the selected background color.",
    fixture: "Particle Grid output with visible gaps between particles",
    id: "export.includeBackground",
    kind: "control",
    target: "export.includeBackground",
    userAction:
      "Turn Include off, verify the live preview background is hidden and exported PNG light pixels are transparent, then turn Include on.",
  },
  {
    automated: true,
    automatedTestName: "background color controls Particle Grid gaps",
    browser: true,
    browserTestName: "browser: background color changes Particle Grid gaps",
    componentType: "color",
    evidence: "rendered-pixels",
    expectedObservable:
      "Changing Background changes gaps around particles in preview and included-background export output.",
    fixture: "Particle Grid output with visible cell gaps",
    id: "appearance.background",
    kind: "control",
    target: "appearance.background",
    userAction: "Choose a non-white Background color and observe the rendered output.",
  },
  {
    automated: true,
    automatedTestName: "image format selects PNG and JPG encoding",
    browser: true,
    browserTestName: "browser: image format changes exported MIME",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "PNG and JPG selections produce matching downloaded MIME types and file extensions.",
    fixture: "uploaded Particle Grid source ready for export",
    id: "export.image.format",
    kind: "control",
    optionCoverage: ["png", "jpg"],
    target: "export.image.format",
    userAction: "Choose PNG and JPG, export each image, and compare MIME type and extension.",
  },
  {
    automated: true,
    automatedTestName: "image resolution selects real 2K 4K and 8K dimensions",
    browser: true,
    browserTestName: "browser: image resolution changes exported dimensions",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "2K, 4K, and 8K selections produce decoded images with 2048, 4096, and 8192 pixel long edges.",
    fixture: "16 by 9 Particle Grid source ready for export",
    id: "export.image.resolution",
    kind: "control",
    optionCoverage: ["2k", "4k", "8k"],
    target: "export.image.resolution",
    userAction: "Choose 2K, 4K, and 8K, export each image, and decode its dimensions.",
  },
  {
    actionCoverage: ["export.png"],
    automated: true,
    automatedTestName: "export action creates Particle Grid image bytes",
    browser: true,
    browserTestName: "browser: export action downloads Particle Grid image bytes",
    componentType: "panelActions",
    evidence: "exported-bytes",
    expectedObservable:
      "Export PNG returns non-empty image bytes using the selected format, resolution, background color, and include-background state.",
    fixture: "uploaded source with visible Particle Grid output",
    id: "actions.output",
    kind: "control",
    target: "actions.output",
    userAction:
      "Upload a source, toggle Include, change Background, choose image Format and Resolution, then click Export PNG and inspect the downloaded bytes.",
  },
  {
    automated: true,
    automatedTestName: "Particle Grid renderer preserves reference shader branches",
    browser: true,
    browserTestName: "browser: Particle Grid renderer produces source-sampled particles",
    componentType: "webgl-renderer",
    evidence: "rendered-pixels",
    expectedObservable:
      "The custom WebGL output samples cell centers and renders luminance geometry, color grouping, deterministic bright-cell dots, and background alpha.",
    fixture: "gradient and saturated color-block source image",
    id: "renderer.particle-grid",
    kind: "runtime",
    referenceCoverage: "renderer-state",
    userAction:
      "Upload the reference fixture and exercise Grid, Particle Shape, Color Mapping, and Dot Pattern branches.",
  },
];
...
export const starterControlSectionInventory: readonly ToolcraftControlSectionInventoryEntry[] = [
  {
    entity: "Source image",
    groupingReason:
      "Upload, preview, clear, reset, and image transforms belong to one source-media workflow.",
    targets: ["source.image"],
    title: "Source",
  },
  {
    entity: "Sampling lattice",
    groupingReason:
      "Cell counts and gaps jointly define the particle sampling grid.",
    targets: gridTargets.map(([target]) => target),
    title: "Grid",
  },
  {
    entity: "Particle mask",
    groupingReason:
      "Width, luminance shrink response, and softness jointly construct each particle shape.",
    targets: shapeTargets,
    title: "Particle Shape",
  },
  {
    entity: "Particle color mapping",
    groupingReason:
      "Original, Tint, grouping mode, and authored group colors define the same particle-color branch.",
    targets: colorTargets.map(([target]) => target),
    title: "Color Mapping",
  },
  {
    entity: "Bright-cell dot texture",
    groupingReason:
      "Chance, background, density, and dot size jointly construct the deterministic sub-pattern.",
    targets: dotTargets.map(([target]) => target),
    title: "Dot Pattern",
  },
  {
    entity: "Output background",
    groupingReason:
      "Inclusion and color jointly define cell gaps and PNG alpha.",
  ...
  },
  {
    ...
    groupingReason:
      "Format and resolution jointly define the downloaded image bytes.",
