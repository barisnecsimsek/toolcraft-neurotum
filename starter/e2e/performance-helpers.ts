...
export async function startToolcraftFrameProbe(
  ...
  await page.evaluate(() => {
    ...
    win.__toolcraftArmFrameProbe = () => {
      ...
      probe.cleanupArm?.();
      probe.longTaskCount = 0;
      probe.longTaskMaxMs = 0;
      probe.maxFrameGapMs = 0;
      probe.sampleCount = 0;
      probe.observer?.takeRecords();
      lastFrame = performance.now();

      const eventNames = ["pointerdown", "keydown", "input", "change", "wheel"] as const;
      const beginInteraction = () => {
        const currentProbe = win.__toolcraftFrameProbe;
        if (!currentProbe?.active) {
          return;
        }

        currentProbe.longTaskCount = 0;
        currentProbe.longTaskMaxMs = 0;
        currentProbe.maxFrameGapMs = 0;
        currentProbe.sampleCount = 0;
        currentProbe.observer?.takeRecords();
        lastFrame = performance.now();
        currentProbe.cleanupArm?.();
      };

      probe.cleanupArm = () => {
        for (const eventName of eventNames) {
          window.removeEventListener(eventName, beginInteraction, true);
        }
        probe.cleanupArm = undefined;
      };

      for (const eventName of eventNames) {
        window.addEventListener(eventName, beginInteraction, true);
      }
    };
...
export async function dragToolcraftSliderByLabel(
  ...
  const sliderRange = await slider.evaluate((element) => {
    ...
    const value = Number(element.getAttribute("aria-valuenow") ?? min);
    return { max, min, value };
  });
  const sliderSpan = sliderRange.max - sliderRange.min;
  const startRatio =
    sliderSpan > 0 ? (sliderRange.value - sliderRange.min) / sliderSpan : 0;
  const startX = box.x + box.width * Math.min(1, Math.max(0, startRatio));
