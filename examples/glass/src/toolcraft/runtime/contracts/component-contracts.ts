...
export const TOOLCRAFT_COMPONENT_CONTRACTS = {
  ...
  vector: {
    ...
    aiUsageRules: [
      ...
      "Double-clicking a vector pad resets both axes to 0,0 through the normal runtime value update; do not add a separate custom reset UI for this basic pad-center behavior.",
  ...
  canvas: {
    aiUsageRules: [
      ...
      "Use fixed-output only when the product output size must not be user-editable because of a real fixed-format, product-defined, or explicitly locked dimension requirement, and prove that lock with canvasSizingCoverage fixed-output-size acceptance.",
      "A reference or previous app lacking a size editor is not a fixed-output reason; product-output clones still use editable-output unless the reference explicitly defines a locked deliverable size.",
