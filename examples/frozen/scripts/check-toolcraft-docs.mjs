...
const forbiddenActiveVerificationGuidancePatterns = [
  /\bthe protected targeted runner\b/i,
  /\b(?:later|ordinary|post-first-working)[^\n]*(?:use|run|record)[^\n]*(?:targeted (?:runner|verification)|(?:current |targeted |baseline-linked )?iteration receipt)/i,
  /app-verification-impact\.json/i,
  /toolcraft-verification-impact(?:\.[cm]?[jt]s)?/i,
  /run-performance-iteration\.mjs/i,
  /verify:perf:record-iteration/i,
];
...
const activeVerificationGuidanceSources = [
  { relativePath: "AGENTS.md", source: agentsSource },
];
...
for (const fileName of [...requiredDocs, ...requiredCoreDocs]) {
  ...
  if (fileName !== "agent-worklog.md") {
    activeVerificationGuidanceSources.push({ relativePath, source });
  }
}
...
for (const { relativePath, source } of activeVerificationGuidanceSources) {
  for (const pattern of forbiddenActiveVerificationGuidancePatterns) {
    if (pattern.test(source)) {
      failures.push(
        `${relativePath} contains unsupported frozen targeted-verification guidance ${pattern}`,
      );
    }
  }
}
