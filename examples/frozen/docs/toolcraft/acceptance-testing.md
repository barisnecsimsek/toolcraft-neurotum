...
`npm run verify:final` must pass before final delivery. This frozen compatibility snapshot predates canonical version-2 ownership: it validates its existing protected checkpoint and legacy baseline-linked receipts but cannot mint a new targeted iteration. Regenerate from the current starter before later product work. Agent-controlled browser checks remain useful for diagnosis but cannot replace protected receipts.

A full performance checkpoint is triggered only by the one-time transition to the first stable working app version, or by a user request to optimize performance, fix lag, remove jank, speed up animation, stabilize drag/zoom, or otherwise investigate poor performance. Existing first-working status is not itself a recurring trigger.
...
Performance changes still update `app-performance.ts`, product performance path adapters, and the kernel candidate harness when assessment requires it. Do not add a local impact inventory or targeted writer to this frozen snapshot; regenerate it from the current starter first.
