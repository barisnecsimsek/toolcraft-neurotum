...
| Tier 4 | Final delivery, fresh export, runtime/template/contract changes, broad renderer/product rewrites | `npm run verify:final`; at the first stable working milestone run protected `npm run verify:perf` first. This frozen legacy snapshot cannot verify later deliveries; regenerate a fresh app from the current starter/CLI. |
