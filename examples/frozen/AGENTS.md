...
17. Performance planning follows one sequence: reachable controls and inputs; workload dimensions and enforced boundaries; pass cost, frequency, lifecycle, and invalidation; render-plan assessment and protected kernel benchmark when required; derived paths and combined fixtures; targeted development checks; first stable full checkpoint. This frozen compatibility snapshot predates canonical version-2 verification ownership and cannot record new targeted iterations; regenerate from the current starter before later product work. Details live in `docs/toolcraft/core/performance.md` and `docs/toolcraft/performance.md`.
...
- Do not add a local impact inventory to this frozen snapshot. Its legacy receipts remain readable, but later product work requires regeneration from the current starter and its canonical version-2 ownership stack.
...
| Tier 4 — final delivery/template architecture | Fresh generated app completion, folder export, commit-ready delivery, dependency changes, runtime/template/contract/CLI changes, broad refactors, or major post-generation iterations that rewrite renderer, canvas, animation, timeline/keyframes, layers, media, export, or control mapping. | Fresh folders run `npm install` once. At the first stable working milestone, run the browser performance checkpoint first and record its durable baseline plus current checkpoint, then run `npm run verify:final`, then start `npm run dev` to provide the local URL. This frozen legacy snapshot does not support current targeted verification ownership; regenerate a fresh app from the current starter/CLI before any later delivery or performance iteration. |
...
Run a full performance checkpoint only at two events: once, after functional work and targeted checks are complete, when the app first reaches a stable working product version; or when the user explicitly asks to optimize performance, fix lag, remove jank, speed up animation, stabilize drag/zoom, or otherwise complains about performance. The first event uses `npm run verify:perf`; explicit performance work after the baseline exists uses `npm run verify:perf:refresh`. A normal later iteration cannot silently refresh the full baseline. Use the current AI agent's controlled browser for targeted diagnosis.

Do not begin a later feature loop in this frozen legacy snapshot. Regenerate a fresh app from the current starter/CLI so its current ownership stack can derive and execute the required targeted checks.

The first stable working product app version is not complete until `npm run verify:perf` has produced the durable full-checkpoint baseline plus current-source receipt and `npm run verify:final` has passed against them. Agent-controlled browser checks remain useful for targeted diagnosis and visual verification, but terminal prose cannot mint the baseline. Worklog prose is context, not execution proof. This frozen compatibility snapshot validates existing baseline-linked receipts but exposes no targeted-iteration writer; regenerate from the current starter before changing the product.
...
For the first stable working product delivery, run `npm run verify:perf` once before `npm run verify:final`; its protected Playwright runner records the durable baseline and current checkpoint. This frozen legacy snapshot does not support current targeted verification ownership; regenerate a fresh app from the current starter/CLI before any later delivery or performance iteration. The final gate rejects a missing baseline or missing, mismatched, or stale current receipt.
