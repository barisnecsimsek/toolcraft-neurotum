...
export async function dragCanvasHandle(
  ...
  const handle =
    typeof testIdOrLocator === "string"
      ? getCanvasHandle(page, testIdOrLocator)
      : testIdOrLocator;
  const handleLabel =
    typeof testIdOrLocator === "string"
      ? `Canvas handle "${testIdOrLocator}"`
      : "Canvas interaction target";

  await expect(handle, `${handleLabel} should be visible`).toBeVisible();

  const box = await handle.boundingBox();
  if (!box) {
    throw new Error(`Could not measure ${handleLabel.toLowerCase()}.`);
  }

  const startX = box.x + (delta.start?.x ?? box.width / 2);
  const startY = box.y + (delta.start?.y ?? box.height / 2);
